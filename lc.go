...
type Letterstatus struct {
	importer			               uint  `json:"importer"`
	importerbank		               uint   `json:"importerbank"`
	exporterbank		               uint   `json:"exporterbank"`
	exporter                           uint  `json:"exporter"`
	exportcustoms			           uint  `json:"exportcustoms"`
	importcustoms			           uint    `json:"importcustoms"`
}
...
func (tradefinance *LC) CreateLC(stub shim.ChaincodeStubInterface, args []string) peer.Response {
    ...
    payment := strconv.Atoi(string(args[6]))
